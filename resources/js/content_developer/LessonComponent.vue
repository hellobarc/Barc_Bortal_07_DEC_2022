            ...
            <div class="row">
                ...
                <div class="col-md-6">
                   <div v-if="module_id==1">
                      <ReadingLessonlist :lesson_list="lesson_list" :module_id="module_id"/>
                   </div>

                   <div v-if="module_id==2">
                       <ListeningLessonlist :lesson_list="lesson_list" :module_id="module_id"/>
                   </div>

                    <div v-if="module_id==5">
                       <GrammarLessonlist :lesson_list="lesson_list" :module_id="module_id"/>
                   </div>

                   <div v-if="module_id==6">
                       <VocabularyLessonlist :lesson_list="lesson_list" :module_id="module_id"/>
                   </div>
                </div>
            </div>
        </main>
    ...
    export default {
           ...
           computed:{
               loggedIn() {
                ...
                module_name(){
                  return  BasicDataService.moduleName(this.module_id);
                },
           },
           methods: {
            handleSubmit(){
                ...
                if(this.selected_unit==0 || this.selected_unit==null || this.selected_unit==""){
                       this.alert_next.push("Please select a Unit");
                       this.iscomplete = false;
                       return;
                }else{
                    this.iscomplete = true;
             ...
             lessonByModule(){
                ContentDataService.lessonByModule(this.module_id,this.auth_token)
                .then(response=>{
             ...
             lessonByModuleCourse(){
                ContentDataService.lessonByModuleCourse(this.selected_course,this.module_id,this.auth_token)
                .then(response=>{
                    this.lesson_list = response.data.data;
                })
                .catch(error=>{
